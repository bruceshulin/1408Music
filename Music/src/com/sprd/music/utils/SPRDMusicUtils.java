...
public class SPRDMusicUtils {
    ...
    public static String makeTimeString4MillSec(Context context, long millSecs) {
        int secs = Math.round((float) millSecs / 1000);
        return MusicUtils.makeTimeString(context, secs == 0 ? 1 : secs);
    }
    ...
    public static int getnuminserted(ContentResolver resolver, Uri uri, String[] cols, int base) {
        ...
        try {
            if (afterInsert != null && afterInsert.moveToFirst()) {
                numinserted = afterInsert.getInt(0) - base;
            }
        } catch (Exception e) {
    ...
    /**
     * SPRD: add for filemanager@{ provider dailog for user to choose. add for
     * filemanager
     * @param context
     * @param playlist
     */
    ...
    /* SPRD: add for delete item , bug 265852 @{ */
    ...
    /* SPRD:add this use to unified manage Activity 399553@{*/
    private static MusicApplication sMusicApplication = MusicApplication.getInstance();
    /* @} */
    /*SPRD:add for bug 257696 add quitservice in the menu*/
    ...
    private static boolean isSimCardExist(Context context, int phoneID) {
        return ((TelephonyManagerSprd)TelephonyManager.from(context)).hasIccCard(phoneID);
    }

    static void updateAudioProfile(Context context, String szPath, int simID) {
        MusicLog.e(LOGTAG, " Audio Path =[" + szPath + "]");
        ContentResolver resolver = context.getContentResolver();
        ContentValues values = new ContentValues(2);
        values.put("ringUri", szPath);
        values.put("ringTwoUri", szPath);
        String szUri = "content://com.thunderst.audioprofile.provider/profiles";
        Uri contexturi = Uri.parse(szUri);
        resolver.update(contexturi, values, null, null);
        MusicLog.e(LOGTAG, "exit insert profile =========");
    }
